...
本轮 Continuation 轨检查了近期 `neo-carve-20260708`、`neo-carve-20260707`
和 `aut-carve-20260625`，没有发现 `## froQ 反馈` 下的新回应，
所以没有生成 Continuation。主轨继续往外走。

近两天 Git 变化中，最明显的是 GLAST 昼夜复合热极端、AIGC badge、
corpus layer 过滤和 tooltip 稳定性。但前两次 Carve 已经分别写过
...
这次 Carve 想把这个判断从操作经验抬升为 corpus 方法论：
...
## 一个判断：Carve 应该先保护中心，再提供增量

Carve 的任务名里有 carve，很容易让 agent 误以为每次必须长出可见枝条。
...
这也解释了为什么 Carve 不能只按「有没有新文件」「有没有新论文」触发。
...
但一旦要追溯为什么某天没有通知、为什么某篇 Carve 没有生成，
它能提供判断的痕迹。

这和 `aut-carve-20260625` 里提到的「蒸馏断裂面」正好互补。
...
这次 Carve 没有从一篇新论文里抽概念，也没有从界面改动里抽设计原则，
而是从大量「无需行动」中辨认出一个系统判断：安静不是缺席，
而是注意力系统里的外围层。

对 Carve 来说，真正要维护的不是输出频率，而是可恢复性。
...
本轮检查近期 Carve / Continuation 文件时，没有发现 `## froQ 反馈` 下有新的有效回应，
因此未生成 Continuation。Carve 方向来自近两天巡检日志中反复出现的
...
Carve 的默认姿态应是外围感知，只有在材料产生明确判断增量或风险时才进入中心。
