...
Carve 则像巡田：不直接替代播种，也不只做库存盘点，而是看哪些枝条
已经露出下一次分株的可能。
...
这可以是原始 capture 链接、文献 citation、日期日志、相关 Carve，或一个明确的
provenance 句子。
...
Corpus 现在已经有 layer、tag、capture、carve、activity、dashboard 这些部件。
...
所以这里可以先不急着立刻修改模板，只把一个轻量规则放在 Carve 枝条上：
...
本轮没有发现新的 froQ 反馈，因此未生成 Continuation。Carve 方向来自近两天
