import type { BoardData, BoardTask, DashboardCalendarData, DashboardCalendarEvent, ScheduleData, ScheduleItem } from '../types'
...
const IGNORED_TASK_STATUSES = new Set(['done', 'cancelled', 'deferred', 'deffered'])

export default defineLoader({
   watch: [BOARD_FILE, SCHEDULE_FILE],

   load(watchedFiles): DashboardCalendarData {
      const boardFile = watchedFiles.find(file => file.endsWith('board.yml'))
         ?? path.join(process.cwd(), BOARD_FILE)
      const scheduleFile = watchedFiles.find(file => file.endsWith('schedule.yml'))
      ...
      const scheduleEvents = (scheduleData.schedule ?? [])
         .map(scheduleItemToCalendarEvent)

      return {
         updated: scheduleData.updated ?? board.updated ?? '',
         events: [...boardEvents, ...scheduleEvents]
...
function boardTaskToCalendarEvent(task: BoardTask, index: number): DashboardCalendarEvent[] {
   if (!task.due)
      return []

   if (task.status && IGNORED_TASK_STATUSES.has(task.status))
      return []
...
function scheduleItemToCalendarEvent(item: ScheduleItem): DashboardCalendarEvent {
   return {
      id: item.id ?? `schedule-${stableHash(`${item.title}-${item.start}-${item.end}`)}`,
      title: item.title,
...
function normalizeDateInput(input: string): { value: string, date: string, hasTime: boolean } {
   const trimmed = input.trim()
   const isoLike = trimmed.replace('T', ' ').replace(/([+-]\d{2}:?\d{2}|Z)$/u, '').trim()
   const dateOnly = isoLike.match(/^(\d{4}-\d{2}-\d{2})$/u)
   if (dateOnly) {
      return { value: `${dateOnly[1]} 00:00`, date: dateOnly[1], hasTime: false }
   }

   const dateTime = isoLike.match(/^(\d{4}-\d{2}-\d{2})\s+(\d{2}:\d{2})/u)
   if (dateTime) {
      return { value: `${dateTime[1]} ${dateTime[2]}`, date: dateTime[1], hasTime: true }
   }
