...
const EXT_RE = /\.(yml|yaml)$/;

declare const data: WeekDashboardData;
export { data };

export default defineLoader({
	watch: ["docs/dashboard/weekTasks/*.yml"],
	...
	load(watchedFiles): WeekDashboardData {
		...
		const baseDir = watchedFiles.length
			? path.dirname(
					watchedFiles[0].split(path.sep).slice(0, -1).join(path.sep),
				)
		...
		const parsedWeeks = weekFiles
			.map((abs) => {
				const filename = path.basename(abs).replace(EXT_RE, "");
		...
		const currentWeek = currentFile
			? safeWeekData(
					readYaml<WeekData>(path.join("docs/dashboard/", currentFile)),
					currentStart,
				)
			: null;
...
function safeWeekData(
	input: Partial<WeekData> & { start?: string },
