...
export default defineLoader({
	...
	load(watchedFiles): VisionsData {
		...
		const yearFiles = fs.existsSync(visionsDir)
			? fs
					.readdirSync(visionsDir)
					.filter((f) => f.startsWith("year-"))
					.map((f) => path.join(visionsDir, f))
			: [];

		const global =
			globalFile && fs.existsSync(globalFile)
				? {
						type: "global" as const,
						items: readYaml<VisionItem[]>(globalFile) ?? [],
					}
		...
		const currentYearVision =
			years.find((y) => y.year === currentYear) ?? years[0] ?? null;
