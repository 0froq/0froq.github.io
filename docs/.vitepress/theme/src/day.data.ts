...
export default defineLoader({
	...
	load(watchedFiles): DayData {
		...
		const days = files
			.map((abs) => {
				const date = path.basename(abs).replace(EXT_RE, "");
				return safeDayRecord(
					readYaml<DayRecord>(abs),
					date,
					`./dayTodos/${path.basename(abs)}`,
				);
			})
		...
		const currentDay =
			days.find((day) => day.date === today) ?? days[0] ?? null;
...
function safeDayRecord(
	input: Partial<DayRecord> & { date?: string },
