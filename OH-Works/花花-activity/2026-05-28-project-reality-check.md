...
- 标题：_Slope-level decoupling reveals staircase warming in global lake thermal trajectories_
...
- **1980s 气候 regime shift 引文**：`#check()[Need a precise citation for the 1980s climate regime-shift rationale]`。项目 `zotero.bib` 中已有 `reid2016`（Reid et al., 2016, _Global Change Biology_ — 正是 1980s regime shift 的标志性综述，31 位作者），也已有 `woolway2017`（Woolway et al., 2017, _Climatic Change_ — 中欧湖泊对 1980s regime shift 的响应）。这两个引用连用即可覆盖 regime shift 理由。

### 需要补充的引文

- **Cahill et al. (2015)** _Change points of global temperature_, Environmental Research Letters, 10, 084002。这篇用无约束 change point 分析检测全球温度记录的断点，结论是 1970 年以来没有 detectable 的 warming trend change。可用于：
